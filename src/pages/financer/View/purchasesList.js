...
function PurchasesList() {
  ...
  const { costs, loading } = useSelector((state) => state.adminData);
  ...
  function onPrint(selectedRow) {
    ...
    setSelectedExpense(selectedRow);
    setInvoiceModalOpen(true);
  }
  function onViewReceipt(selectedRow) {
    setSelectedExpense(selectedRow);
    console.log(selectedExpense.receipts.split(','));
      ...
      renderCell: (params) => {
        return (
          <div className='w-full flex justify-center'>
            <BootstrapTooltip title='Receipt'>
              <IconButton
                size='small'
  ...
  return (
    <>
      <Grid container className='dashboard-container justify-around'>
        <Grid className='accounts-list-container w-full -mt-3'>
          <div className='w-full flex flex-row justify-between mb-2'>
            <p className='h4 text-left'>Costs/Purchases</p>
          </div>
          <DataGrid
      ...
      <Modal open={receiptModalOpen} onClose={() => setReceiptModalOpen(false)}>
        <div
          className='h-screen w-fit overflow-auto'
          style={{ marginLeft: '15%' }}
        >
          {selectedExpense ? (
            <Grid className='invoice-layout-container w-4/5 mx-auto bg-white overflow-hidden p-6'>
              {selectedExpense.receipts.split(',').map((receipt, index) => (
                <img key={index} src={receipt} alt={receipt} />
              ))}
            </Grid>
          ) : null}
        </div>
